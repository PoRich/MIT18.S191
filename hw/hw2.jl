...
# ╔═╡ 07191f40-1560-11eb-006f-cf5f811ed927
using Logging
...
1029 to 3 allocs; no longer compying each pixel when creating the new image
"""
...
function greedy_seam(energies, starting_pixel::Int)
	@debug "input size: $(size(energies)) starting_pixel : $(starting_pixel)"
	n_rows, n_cols = size(energies)
	path = zeros(Int, n_rows) # initialize container for optimal path
	path[1] = starting_pixel
	for i in 2:n_rows
		current_pixel = path[i-1]
		# possible moves in NEXT row based on current position
		left = max(1, current_pixel-1)  # edge case A
		right = min(n_cols, current_pixel+1) # edge case B
		# next move needs to map to (-1, 0, 1) 
		path[i] = current_pixel + findmin(energies[i, left:right])[2] -2
		# however in edge Case A, next move can only be (0, 1)
		if left == 1
			path[i] +=1
		end
		@debug "row:$(i) l:r: $(left):$(right) neighbors: $(energies[i, left:right]) next step: $(path[i])"
	end
	return path
end

# ╔═╡ 6f210a32-156d-11eb-3168-89dc280234db


# ╔═╡ aed483bc-156c-11eb-2d42-35d3d485604a
findmin([Gray{Float64}(0.8870091781767242), Gray{Float64}(0.22840592375992275)])
...
function least_energy(energies, i, j)
	...
	# base case: last row
	if i == r
		#@debug "A0-BASE @ i,j $(i),$(j) TOTAL ENERGY = $(energies[i,j])"
		return (energies[i,j], j)
	end

	# induction
	#@debug "A1-CALLED W size-$(size(energies))energy @ i,j $(i),$(j)"
	left, right = max(1, j-1), min(c, j+1)
	next_energy, next_step = findmin(energies[i+1, left:right])
	next_idx = j+next_step-2  # (-1, 0, 1) 
	if left ==1  # handle edge case 
		next_idx +=1
	end 
	
	#@debug "A2 left:right $(left):$(right), neighors $(energies[i+1, left:right])"
	total_energy = energies[i,j] + least_energy(energies, i+1, next_idx)[1]
	#@debug "A4-RECURSE @ i,j $(i),$(j) TOTAL ENERGY = $(total_energy) = $(energies[i,j]) + $(next_energy)"
	return (total_energy, next_idx)
end
...
function recursive_seam(energies, starting_pixel)
	...
	for i in 2:r
		next_step = least_energy(energies, i, seam[i-1])[2]
		@debug "B1- next_step @row $(i) $(next_step)"
		seam[i] = next_step
	end
	return seam
end
...
At every row, the algorithm considers 3 new forks for each step; Big O is n_rows^3 so it is exponential
"""
...
# ╔═╡ b880bfa2-160c-11eb-339d-45851a7dad79
begin
	# dictionary test
	test_dict = Dict("a"=>54, "b"=> 985, "c"=> 12)  #define dictionary
	test_dict2 = Dict{String, Int}("a"=>54, "b"=> 985, "c"=> 12)  #define dictionary
	test_dict3 = Dict(i=>i^3 for i = 11:20) #define dictionary
	test_dict3[13] #lookup
	test_dict["a"]=9876  #set value
end

# ╔═╡ 26a3b032-160f-11eb-12f1-af693c016ecd
test_dict["a"]

# ╔═╡ b1d09bc8-f320-11ea-26bb-0101c9a204e2
function memoized_least_energy(energies, i, j, memory)
	if haskey(memory, (i,j)) == false
		memory[(i,j)] = least_energy(energies, i, j)
	end
	return memory[(i,j)]
end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	for i in 2:r
		next_step = memoized_least_energy(energies, i, seam[i-1], memory)[2]
		@debug "C1- next_step @row $(i) $(next_step)"
		seam[i] = next_step
	end
	return seam
end
...
function matrix_memoized_least_energy(energies, i, j, memory)
	m, n = size(energies)
	if memory[i,j] == 0
		memory[i,j] = least_energy(energies, i, j)[2]
	end 
	return memory[i,j]
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	for i in 2:r
		next_step = matrix_memoized_least_energy(energies, i, seam[i-1], memory)
		@debug "D1- next_step @row $(i) $(next_step)"
		seam[i] = next_step
	end
	return seam
...
function least_energy_matrix(energies)
	r, c = size(energies)
	energy_map, next_steps = zeros(r,c), zeros(Int, r, c)
	energy_map[end,:] = energies[end,:]
	for i = r-1:-1:1, j = 1:c
		left, right = max(1,j-1), min(c,j+1)
		@debug "r:$(i), c:$(j) l:$(left), r:$(right)"
		local_energy, next_step = findmin(energies[i+1, left:right])
		energy_map[i,j] = energies[i,j] + local_energy
		next_steps[i,j] = next_step - 2
		if left == 1 
			next_steps[i,j] += 1
		end
	end
	return energy_map, next_steps
end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	energy_map, next_steps = least_energy_matrix(energies)
	m, n = size(next_steps)
	seam = zeros(Int, m)
	seam[1] = starting_pixel
	for i in 2:m
		seam[i] = seam[i-1] + next_steps[i-1, seam[i-1]]
	end
	return seam
end
...
# ╔═╡ fff470dc-15ed-11eb-3184-65c7c5e015f7
least_energy(energy(pika),1,7)

# ╔═╡ 30762b74-15ee-11eb-21ab-19a70687c982
energy(pika)[:,6:end]

# ╔═╡ 5b2c93e8-1600-11eb-0cac-9bbb1962f2c9
begin
	r, c = size(pika)
	seam = zeros(Int, r)
	seam[1] = 5
	for i in 2:r
		next_step = least_energy(energy(pika), i, seam[i-1])[2]
		#@debug "B1- next_step @row $(i) $(next_step)"
		seam[i] = next_step
	end
end

# ╔═╡ a88ab320-1605-11eb-041d-714023bb5833
r, seam
...
# ╔═╡ 0cfcd7ac-1610-11eb-3848-2b62b41cb301
begin
	memory = Dict{Tuple{Int,Int}, Tuple{Float64, Int}}()
	memory[(1,7)] = least_energy(energy(pika), 1, 5)
	memory[(1,7)]
end

# ╔═╡ b27cb5be-179e-11eb-12ab-e17c6492980a
recursive_memoized_seam(energy(pika), 3)

# ╔═╡ ba9043ca-17a2-11eb-106c-a3868fd14c98
matrix_memoized_seam(energy(pika), 3)

# ╔═╡ 5fdec3ba-17aa-11eb-1af0-239cf0ec1616
energy(pika)

# ╔═╡ c636b968-17a5-11eb-37c5-0d4e3ca9de25
least_energy_matrix(energy(pika))

# ╔═╡ c5f4fc5e-17a9-11eb-2b7d-a922cc68de45
seam_from_precomputed_least_energy(energy(pika), 4)
