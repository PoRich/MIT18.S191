...
using StatsBase
	function frequencies(values)
		...
		return StatsBase.countmap(values)
	end
end
...
@bind p_interactive Slider(0.01:.01:1; default=0.25, show_value=true)

# ╔═╡ 3ded8932-1aad-11eb-3da3-33939ecaf56f
@bind N_interactive Slider(1:10_000, default=5000, show_value=true)

# ╔═╡ 03231438-1aae-11eb-2d9d-3f006f3db017
...
# ╔═╡ 3a3fb496-1ab1-11eb-1231-c191533bb8e6
let
λ = 2.5
p = plot()
x1 = 1:.01:10; y1 = (λ*ℯ).^-(λ*x1);
plot!(p, x1, y1)
end
...
md"""Closely resembles exponential distribution"""
...
begin
	n = 10_000
	xs =0.001:0.01:1
	results = [mean(do_experiment(p, n)) for p ∈ xs]
	plot(xs, results, title="Avg Days to Recovery")
	ylabel!("Avg Days to Recover")
...
# ╔═╡ b602e5b4-1ab4-11eb-3a7e-9d4d36745b75
Int(S), Int(I), Int(R)
...
"""Generates N agents with 1 random agent infected"""
function generate_agents(N::Integer)
	
	infected_idx = rand(1:N)
	agents = [Agent() for i in 1:N]
	agents[infected_idx].status = I
	return agents
end
...
mutable struct InfectionRecovery <: AbstractInfection
	p_infection
	p_recovery
end
...
# ╔═╡ 754451be-1eb7-11eb-28be-29bfc6c67d0e
function plot_repeated_sims(simulation)
	...
	plot!(p, i_mean, lw=3, label="average infections")
end
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	n_sims, n_time = length(simulations), length(first(simulations).S)
	# convert simulations (named tuple of arrays into matrix of infections)
	s_mtx, i_mtx, r_mtx = zeros((n_sims, n_time)), zeros((n_sims, n_time)), zeros((n_sims, n_time))
	for s in 1:n_sims
		s_mtx[s, :] = simulations[s].S  # susceptible  matrix
		i_mtx[s, :] = simulations[s].I  # infection  matrix
		r_mtx[s, :] = simulations[s].R  # reinfection  matrix
	end
	# calculate the mean for each time period 
	s_mean, i_mean, r_mean = zeros(n_time), zeros(n_time), zeros(n_time)
	for t in 1:n_time
		s_mean[t], i_mean[t], r_mean[t] = mean(s_mtx[:,t]), mean(i_mtx[:,t]), mean(r_mtx[:,t])
	end
	p = plot()
	plot!(p, 1:1000, s_mean, alpha=0.5, label="mean susceptible")
	plot!(p, 1:1000, i_mean, alpha=0.5, label="mean infected")
	plot!(p, 1:1000, r_mean, alpha=0.5, label="mean recovered")
	return p
end
...
# ╔═╡ 1d16adb6-1d33-11eb-0154-13902e155a95
begin
infect_slider = @bind p_infection Slider(0.01:.005:.2, show_value=true)
recovery_slider = @bind p_recovery Slider(0.001:.0005:.1, show_value=true)

	md"""
	infection rate $(infect_slider)
	
	recovery rate $(recovery_slider)
	"""
end
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	# calculate the mean for each time period 
	s_mean, i_mean, r_mean = zeros(n_time), zeros(n_time), zeros(n_time)
	s_stdev, i_stdev, r_stdev = zeros(n_time), zeros(n_time), zeros(n_time)
	for t in 1:n_time
		s_mean[t], i_mean[t], r_mean[t] = mean(s_mtx[:,t]), mean(i_mtx[:,t]), mean(r_mtx[:,t])
		s_stdev[t], i_stdev[t], r_stdev[t] = Statistics.std(s_mtx[:,t]), Statistics.std(i_mtx[:,t]), Statistics.std(r_mtx[:,t])
	end
	p = plot()
	plot!(p, 1:1000, s_mean, alpha=0.25, ribbon=s_stdev, label="mean S")
	plot!(p, 1:1000, i_mean, alpha=0.25, ribbon=i_stdev, label="mean I")
	plot!(p, 1:1000, r_mean, alpha=0.25, ribbon=r_stdev, label="mean R")
	return p
end

# ╔═╡ c9acbfec-1ea4-11eb-0662-d9d8e57619b1
function sir_mean_error_plot2(simulations::Vector{<:NamedTuple})
	n_sims = length(simulations)
	xs = 1:1000
	# mtx_s is size(1000, 20) - rows are time, cols are simulations
	mtx_s = reduce(hcat,[sim.S for sim in simulations])
	mtx_i = reduce(hcat,[sim.I for sim in simulations])
	mtx_r = reduce(hcat,[sim.R for sim in simulations])
	#sum across dims=2 collapses the 2nd dim resulting in size(1000,1) array
	mean_s = sum(mtx_s, dims=2) ./n_sims 
	mean_i = sum(mtx_i, dims=2) ./n_sims
	mean_r = sum(mtx_r, dims=2) ./n_sims
	std_s = (sum((mtx_s .-mean_s).^2, dims=2) ./ (n_sims-1)) .^0.5
	std_i = (sum((mtx_i .-mean_i).^2, dims=2) ./ (n_sims-1)) .^0.5
	std_r = (sum((mtx_r .-mean_r).^2, dims=2) ./ (n_sims-1)) .^0.5
	p = plot()
	plot!(p, xs, mean_s, alpha=0.25, ribbons=std_s, label="mean S")
	plot!(p, xs, mean_i, alpha=0.25, ribbons=std_i, label="mean I")
	plot!(p, xs, mean_r, alpha=0.25, ribbons=std_r, label="mean R")
	return p
...
mutable struct Reinfection <:AbstractInfection
	p_infection
	p_recovery
end
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	...
	a1_idx, a2_idx = 1, 1 
	while a1_idx == a2_idx  # need to insure agent1 != agent2
		a1_idx, a2_idx = rand(1:n_agents), rand(1:n_agents)
	end
	a1, a2 = agents[a1_idx], agents[a2_idx]
	interact!(a1, a2, infection)
	return agents
end
...
let
	...
	# convert simulations (named tuple of arrays into matrix of infections)
	n_sims, n_time = length(simulations), length(simulations[1].I)
	i_matrix = zeros((n_sims, n_time))
	for s in 1:n_sims
		i_matrix[s, :] = simulations[s].I  # infection  matrix
	end
	# cacluate the mean for each time period 
	mean_infections = [mean(i_matrix[:,t]) for t in 1:n_time]
	plot!(p, 1:1000, mean_infections, lw=3, label="mean infections")
	p
end

# ╔═╡ 9c49ce98-1eb8-11eb-3653-5f8ed229b791
plot_repeated_sims(simulations)

# ╔═╡ 7f635722-04d0-11eb-3209-4b603c9e843c
sir_mean_plot(simulations)

# ╔═╡ 77d45862-1d35-11eb-13f1-1b3a1235a295
begin
	simulations2 = repeat_simulations(1000000, 1000, InfectionRecovery(p_infection, p_recovery), 20)
	sir_mean_plot(simulations2)
end

# ╔═╡ a9712c3e-1eab-11eb-3156-7583f9cb061f
sir_mean_error_plot2(simulations2)

# ╔═╡ c2795808-1d3b-11eb-2aa1-fb36fb933038
sir_mean_error_plot(simulations2)

# ╔═╡ 1835e7e8-1d37-11eb-1951-a96773c6f5c3
let
	...
	num_infected = [agent.num_infected for agent in agents]
	p = bar(frequencies(num_infected))
	vline!(p, [mean(num_infected)], label="mean")
end
...
let
	A = 100 # number of agents
	N = 20 # number of trials
	T = 1000 # 
	re_sim = repeat_simulations(A, T, Reinfection(0.02, 0.002), N)
	plot_repeated_sims(re_sim)
	#md""" infections plateu but do not decline"""
end
