...
student = (name = "Rick", kerberos_id = "PoorRichRE")
...
# ╔═╡ 94902f86-25d1-11eb-2ca5-15b18e3092e4
let # exploration of init_rays
	#(400, 300) camera resolution returns 300 x 400 array of photons
	cam = test_cam
	aspect_ratio = cam.resolution[1] / cam.resolution[2]
	dim = (cam.aperture_width, cam.aperture_width/aspect_ratio)  # (9, 6.75)
	xs = LinRange(-0.5* dim[1], 0.5 * dim[1], cam.resolution[1])  #400 elements
	ys = LinRange(0.5* dim[2], -0.5 * dim[2], cam.resolution[2])  #300 elements
...
function init_rays(cam::Camera)
	...
	Photon.([cam.p], directions, [zero(RGB)], [1.0]) # returns an array of photons of shape size(directions)
end

# ╔═╡ 156c0d7a-2071-11eb-1551-4f2d393df6c8
tiny_resolution_camera = Camera((4,3), 16, -5, [0, 20, 100])

# ╔═╡ 8f2af5fa-25d4-11eb-1d2e-4d5bb1cac70a
...
function interact(ray::Photon, hit::Intersection{SkyBox}, ::Any, ::Any)
	# if photon hits SkyBox, return the color 
	ray_color = hit.object.c(hit.point, hit.object)
	return Photon(hit.point, ray.l, ray_color, ray.ior)
end

# ╔═╡ 086e1956-204e-11eb-2524-f719504fb95b
# if photon hits nothing, return the itself
interact(photon::Photon, ::Miss, ::Any, ::Any) = photon

# ╔═╡ d1bfa316-25de-11eb-3efe-2f37a9b070f4
md"""
#### Sphere interaction
"""

# ╔═╡ a0777a26-25df-11eb-1156-d565e4ec0d34


# ╔═╡ e492fdc4-25f9-11eb-3b64-17055c3d699c
begin 
	...
	function interact(ray::Photon, hit::Intersection{Sphere}, num_intersections::Integer, objects::Vector{O}) where {O <: Object}
		reflect_r, refract_r = ray, ray
		s_normal = sphere_normal_at(hit.point, hit.object)
		
		if hit.object.s.r != 0 # reflective
			# temp photon with reflected direction for next recursive call of step_ray
			temp = Photon(hit.point, reflect(ray.l, s_normal), ray.c , ray.ior)  
			# return reflected direction & color of the next hit object
			reflect_r = Photon(hit.point, reflect(ray.l, s_normal), step_ray(temp, objects, num_intersections-1).c, ray.ior)
		end
		
		if hit.object.s.t != 0 # refraction / transmission
			if ray.ior == 1.0  # photon is in air, entering sphere
				new_ior = hit.object.s.ior
			else  # photon is in sphere, entering air 
				new_ior = 1.0
			end
			# temp photon with refracted direction for next recursive call of step_ray
			l2 = refract(ray.l, s_normal, ray.ior, new_ior)
			temp = Photon(hit.point, l2, ray.c , new_ior) # this color is temporary
			# return refracted direction & color of the next hit object
			refract_r = Photon(hit.point, l2, step_ray(temp, objects, num_intersections-1).c, new_ior)
		end
		
		# payload is a photon with weighted avg color of reflected, transmission/refracted, color
		return_color = reflect_r.c * hit.object.s.r + refract_r.c * hit.object.s.t + hit.object.s.c.alpha * hit.object.s.c
		return Photon(ray.p, zeros(length(ray.l)), return_color, ray.ior)
	end
...
# ╔═╡ d7af98fc-25f5-11eb-184c-89becc8ff8f8
@bind N_steps Slider(1:1:10, default = 2, show_value=true)

# ╔═╡ 79ddb062-25f6-11eb-1298-b10d738d48c1
@bind z_zoom Slider(-1000:1:1000, default = 300, show_value=true)

# ╔═╡ e513d154-25e2-11eb-02ba-ad7f147d9798
let  # test Photon, Sphere interaction
	basic_camera = Camera((600,300), 16, -5, [0,0,z_zoom])
	r = 50
	#upper right
	mirror_ball1 = Sphere([300.0, 300.0, 50], r, Surface(1.0, 0.0, RGBA(.6,.6,.3,0), .75))
	#lower right
	mirror_ball2 = Sphere([200.0, -200.0, -50.0], 2r, Surface(1.0, 0.0, RGBA(0,1,0,0), 1.5))
	#upper left
	mirror_ball3 = Sphere([-200, 200.0, 100.0], r, Surface(0.0, 0.0, RGBA(.4,.75,0,1), 1.25))
	# lower left
	mirror_ball4 = Sphere([-125.0, -125.0, 100.0], r, Surface(0.0, 1.0, RGBA(.5,.23,0,0), 1.75))
	#middle ball
	mirror_ball5 = Sphere([0.0, 0.0, -50.0], 1.75*r, Surface(1.0, 0.0, RGBA(1,1,1,0), 1.25))
	scene = [sky, mirror_ball1, mirror_ball2, mirror_ball3, mirror_ball4, mirror_ball5]

	ray_trace(scene, basic_camera; num_intersections=N_steps)
end
