...
student = (name = "Rick", kerberos_id = "PoorRich")
...
Submission by: **_$(student.name)_** ($(student.kerberos_id)@google.com)
"""
...
struct Wall <: Object
	...
	"Normal vector"  # unit length tangent
...
# ╔═╡ 7bacc1f2-2453-11eb-0f28-7159a23746f7
#=deconstruct wall struct
adjacent = [1, 3] # rotating 90 degrees by swaping coordinates, making one negative
a = [8,-1] + [1,3] * 20 = [28,59]  # adding 20x perpendicular; line starting point
b = [8,-1] - [1,3] * 20 = [-12,-61]  # subtracting 20x perpendicular; ending point
...
function intersection_distance(photon::Photon, wall::Wall)
	# returns the time it takes for the photon to collide with the wall
...
function intersection(photon::Photon, wall::Wall; ϵ=1e-3)
	D = intersection_distance(photon, wall)  # time to intersect
	ϵ = 1e-3 # threshold for determining intersection
	if D > ϵ # intersection 
		collision_pt = photon.p + D * photon.l  # intersection point 
		return Intersection(wall, D, collision_pt)
	else
		a_miss = Miss()
		return a_miss
	end
...
function interact(photon::Photon, hit::Intersection{Wall})
	# returns a new photon reflected off the wall
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	...
	if discriminant < 0  # no intersection (solution involves complex numbers)
		a_miss = Miss()
		return a_miss		
	elseif discriminant == 0 # one intersection (tangent)
		t = (-b + discriminant^(0.5))/(2*a)  # solve for time to intersection
		# only return points in forward time (in front of the photon)
		if t > ϵ
			collision_pt = photon.p + t * photon.l  # intersection point 
			return Intersection(sphere, t, collision_pt) 
		else  # the point is behind the photon
			a_miss = Miss()
			return a_miss
		end
	else  # discriminant > 0 means two intersections 
		# solve for time to intersections
		t1, t2 = (-b + discriminant^(0.5))/(2*a), (-b - discriminant^(0.5))/(2*a)
		if t2 < t1  # set t1 to be the smaller of the two
			t1, t2 = t2, t1
		end
		# only return points in forward time (in front of the ray)
		positive_ts = [t for t in [t1,t2] if t > ϵ]
		t_collision_tuples = [(t, photon.p + t * photon.l) for t in positive_ts]
		intersections = [Intersection(sphere, t, pts) for (t, pts) in t_collision_tuples]
		if length(intersections) > 0 # if there is at least one collision in front of the ray
			return intersections[1]  # return the first collision
		else  # if both intersection points are behind the ray  
			a_miss = Miss()
			return 	a_miss
		end
	end
...
# ╔═╡ fa6ea128-2461-11eb-2fc3-df2871e57b24
let
	ex_hit = all_intersections[1]
	ex_hit.object.normal
end
...
function closest_hit(photon::Photon, objects::Vector{<:Object})
	return minimum(intersection(photon, object) for object in objects)
end
...
function interact(photon::Photon, hit::Intersection{Sphere})
	...
	if photon.ior == 1.0 # photon in air, currently entering sphere
		new_ior = hit.object.ior
		l_2 = refract(photon.l, normal, photon.ior, new_ior)
		return Photon(hit.point, l_2, new_ior)
	else # photon in sphere, currently entering air
		new_ior = 1.0
		l_2 = refract(photon.l, normal, photon.ior, new_ior)
		return Photon(hit.point, l_2, new_ior)
	end
...
function step_ray(photon::Photon, objects::Vector{<:Object})
	...
	interact(photon, hit) # returns a new photon 
end

# ╔═╡ 1a43b70c-1ca3-11eb-12a5-a94ebbba0e86
function trace(photon::Photon, scene::Vector{<:Object}, N)
	payload = accumulate(1:N, init=photon) do prev_photon, _
		step_ray(prev_photon, scene) # returns a new photon 
	end
	return payload
end
...
@bind ior_test Slider(0.25:0.01:2, default=1.35, show_value=true)

# ╔═╡ 9415a9ee-2518-11eb-3519-db78756ac390
@bind ray_steps Slider(1:1:30, default=4, show_value=true)

# ╔═╡ 9b3215b2-251c-11eb-1718-ef1ce9ebc721
let
	# set the scene 
	test_lens = Sphere([0,0], 5, ior_test,)
	L = 15  # plot size
	box_scene = [
	Wall([L,0],[-1,0]),
	Wall([-L,0],[1,0]),
	...
	scene = [test_lens, box_scene...]
	# plot the scene
	p = plot_scene(scene, legend=false, xlim=(-(L+1),(L+1)), ylim=(-(L+1),(L+1)))
	# create photons
	i=4
	photons = [Photon([-L,y], [1,0], 1.0) for y in -4:.5:4]
	for i in 1:length(photons)
		path = accumulate(1:ray_steps; init=photons[i]) do old_photon, _
		step_ray(old_photon, scene)
		end
		#@debug "path $(path)"
		line = [photons[i].p, [r.p for r in path]...]
		plot!(p, first.(line), last.(line), lw=0.5, color=:red)		
	end
...
# ╔═╡ 507dc5fa-2451-11eb-1a99-11645fe98e02
dot([8,-1],[-3,1])
