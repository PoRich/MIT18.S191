...
student = (name = "Ricky McRickster", kerberos_id = "PoorRichre")
...
# derive A by setting absorbed_solar_radiation = outgoing_thermal_radiation (corresponding to the fact that pre-industrial temperature was constant at T0)
...
function timestep!(ebm)
	append!(ebm.T, ebm.T[end] + ebm.Δt*tendency(ebm));  # update temperature
	append!(ebm.t, ebm.t[end] + ebm.Δt);  # time stepping
end;
...
begin
	mutable struct EBM
		T::Array{Float64, 1}  # array of temperatures
	
		t::Array{Float64, 1}  # array of time t years
		Δt::Float64
...
A more negative B, means a greater amount of heat is radiated from Earth into space (blackbody radiation); this is a feedback parameter because they dampen warming (and hence have a negative value
"""
...
B=0 implies no blackbody radiation feedback; B>0 means radiation is a positive feedback loop creating more radiation
"""
...
# ╔═╡ aee2c714-2779-11eb-1e02-bb32c25f8343
let
	t = 1850
	while Model.CO2_RCP85(t) < 280*2
		t +=1
	end
	t
end
...
# ╔═╡ 9277a3ba-28d5-11eb-262b-2fbcfb57a015
md"**Answer:**"

# ╔═╡ 9eb27e66-28d2-11eb-25c7-6735461d4ab1
let  # approach #1
	ECS_samples = []
	...
	for b in B_samples
		# the definition of A depends on B, so we recalculate:
		A = Model.S*(1. - Model.α)/4 + b*Model.T0
		
		# create the model
		ebm_ECS = Model.EBM(14., 0., 1., double_CO2, A=A, B=b);
		Model.run!(ebm_ECS, 300)
		push!(ECS_samples, last(ebm_ECS.T)-first(ebm_ECS.T))
	end		
	histogram(ECS_samples, size=(600, 250), label=nothing, xlabel="ΔT(t) = T(300) - T₀", ylabel="samples")
end

# ╔═╡ 4b7946fc-28d8-11eb-0606-2da02add5ac6
ECS_samples = let  # approach #2
	[ECS(B=b) for b in B_samples]
end

# ╔═╡ 82c44148-28d8-11eb-1783-b7da6e6faeb1
histogram(ECS_samples, size=(600, 250), label=nothing, xlabel="ΔT(t) = T(300) - T₀", ylabel="samples", xlim=(0,17.5))
...
let
	ecs_bbar_mean = ECS(B=mean(B_samples))
	mean(ECS_samples) - ecs_bbar_mean # higher expected warming by 0.4387 degrees
	sum(1 for s in ECS_samples if s > ecs_bbar_mean) / length(B_samples)
	# 50.54% chance ECS(B) > ECS(B̄)
end
...
Worse, it implies higher expected warming by 0.4387 degrees
"""
...
let
p = plot()
plot!(p, simulated_rcp85_model.t, simulated_rcp85_model.T)  
	# temperature in 2100 is 17.57 C
end
...
# ╔═╡ 59a94f20-28dc-11eb-29c3-5bf09c4f6f00
low_emissions = let
	temp_resp = [temperature_response(Model.CO2_RCP26, b) for b in B_samples]
	sum([1 for s in temp_resp if s-14. > 2]) / length(B_samples)
end

# ╔═╡ ab33868a-28dc-11eb-322c-41cfe8043e84
high_emissions = let
	temp_resp = [temperature_response(Model.CO2_RCP85, b) for b in B_samples]
	sum([1 for s in temp_resp if s-14. > 2]) / length(B_samples) 
end
...
function step_model!(ebm::Model.EBM, CO2::Real)
	ebm.t, ebm.T = [first(ebm.t)], [first(ebm.T)]
	ebm.CO2 = t -> CO2
	Model.run!(ebm)
	return ebm
end
...
@bind log_CO2 Slider(log(10,CO2min):round(log(10,CO2max)))
...
let
	...
	# your code here 
	ebm = step_model!(Model.EBM(Tneo, 0., 5., Model.CO2_const), 10^log_CO2)
...
# ╔═╡ 04d9de4a-28f2-11eb-3c76-796675cdc62d
...
# ╔═╡ b96e9f08-28f2-11eb-3f6b-67a64f0edef0
equilibrium_temperature(1000000)  # test 

# ╔═╡ 9eb07a6e-2687-11eb-0de3-7bc6aa0eefb0
co2_to_melt_snowball = let
	water_ball_temp = 10
	CO2_range = 10^5:10:10^6
	c02_pt = missing
	for c in CO2_range
		if equilibrium_temperature(c) >= water_ball_temp
			c02_pt = c
			break
		end
	end
