...
# ╔═╡ f9a4ff08-1886-11eb-3850-910d6e3e2d57
typeof(first_sample)
...
unused_letters = let
	function iszero(x)  return x ==0 end
	[alphabet[x] for x in findall(iszero, sample_freqs)]
end
...
# ╔═╡ 5ccae3be-1872-11eb-1de7-97f7887584ae
sample_freqs
...
double_letters = [alphabet[x] for x in findall(x-> x>0,[sample_freq_matrix[i,i] for i in 1:size(sample_freq_matrix)[1]])]
...
row_col_answer = md"""
sum of each row = probability of a word pair ends in the letter corresponding to that row
sum of each col = probability of a word pair begins in the letter corresponding to that column
"""
...
function sample_text(A, n)
	...
	first_index = rand_sample(vec(sum(A, dims=1))) # randomly sample first letter based on frequency of it being the first in a word pair 
	
	indices = reduce(1:n; init=[first_index]) do word, _
		# randomly samples n first words as word variable
		prev = last(word)
		freq = normalize_array(A[prev, :])  # get frequency of second letter
		next = rand_sample(freq)  # randomly sample second letter 
		[word..., next]  # string first and second letters together?
	end
	
	String(alphabet[indices])  # convert indices to letters into String
end

# ╔═╡ 5726bf2e-1879-11eb-1f2d-69ca9cdac30f
#=  #sandbox
begin
	A = transition_frequencies(clean(ex23_sample))
	vec(sum(A, dims=1))
	indices = reduce(1:400; init=[rand_sample(vec(sum(A, dims=1)))]) do word, _ 
		prev = last(word)
		prev
		A[prev, :]
		#A[5, :]
...
function word_counts(words::Vector)
	counts = Dict()
	unique_words = String.(Base.Set(words))
	return Dict(k => count.(k, join(words, " ")) for k in unique_words)
end
...
function completions_cache(grams)
	cache = Dict()
	map(grams) do x
		if haskey(cache, x[1:2])
			append!(cache[x[1:2]], [x[3]])
		else
			cache[x[1:2]] = [x[3]]
		end
	end
...
# ╔═╡ 6c8f3b80-1893-11eb-266a-cd9528c846a0
#=
begin  # sandbox
	sample2="""to be or not to be"""
	ngrams_circular(splitwords(sample2), 5)
	n = 3
	ngrams([splitwords(sample2)..., splitwords(sample2)[1:n-1]...], n)
end
=#
