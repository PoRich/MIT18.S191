...
# ╔═╡ f798ac56-1487-11eb-0593-ef3227a71083
using LinearAlgebra

# ╔═╡ c3e46a6c-1489-11eb-3375-23d9abff9052
using Distributions

# ╔═╡ 3bf8a0a4-14d5-11eb-3b18-abe161a10118
using InteractiveUtils
...
student = (name = "Richie", kerberos_id = "jazz")
...
random_vect = rand(0.:3.,10) # replace this with your code!
...
function vecvec_to_matrix(input)
	rows = length(input)
	cols = maximum([length(input[x]) for x in 1:rows])
	M = zero(rand(rows, cols))
...
function mean_colors(image)
	r, b, g = red.(image), blue.(image), green.(image)
	return RGB(mean(Iterators.flatten(r)), mean(Iterators.flatten(g)), mean(Iterators.flatten(b)))
end
...
begin
	...
	function quantize(image::AbstractMatrix)		
		return reshape([quantize(pixel) for pixel in image],(size(image)))
	end
end
...
begin
	...
	function noisify(x::Number, s)
		#-s is min , s is max->  min + random*(max-min)
		r = rand()
		e = -s + r * 2*s # scales between -s and s(uniformly distr random number)
		return my_clamp(x+e)
	end
	...
	function noisify(image::AbstractMatrix, s)
		# you will write me in a later exercise!
		return [noisify(p, s) for p in image]
	end
end

# ╔═╡ c59b17fa-1463-11eb-0c55-d31118f1acd9
noisify(.5, .2)
...
The image is unrecognisable with intensity ... 3; because it causes enough pixels to go beyond the range of 0:1 that results in an unrecognizable image
"""
...
# ╔═╡ f4281f32-13c8-11eb-32de-bba3fdb566cf
begin
	r, c = size(philip)
	#philip[r÷2, c÷2].b
	r_mean = [mean(i) for i in 1:r]
	mean(r_mean)
	typeof(philip)
end
...
philip_inverted = [invert(p) for p in philip]
...
function blur_1D(v, l)
	extended_v = [extend(v, i) for i in min(-l,0):max(l, length(v))]
	blur_val = mean(extended_v)
	return [blur_val for j in -l:l]
end

# ╔═╡ 2340c23a-1476-11eb-2e7a-a1dbaf8c7a96
[extend(v,i) for i in -20:20]
...
# ╔═╡ 8fa6fed6-1477-11eb-2588-f1aedaa38fcb
@bind window_size html"<input type=range>"

# ╔═╡ 132134de-1476-11eb-16fb-8d1c84dcf6a8
colored_line(blur_1D(v,window_size))

# ╔═╡ b3c1a008-1477-11eb-0ac7-dd21e199e1db
md"""window size: $(window_size)"""
...
function convolve_vector(v, k)
	l = (length(k) - 1) ÷ 2  # kernel window runs from -l to l 
	length(k) %2 == 0 ? adj=1 : adj=0 # adjust factor if window size is even 
	extended_v = [extend(v, i) for i in 1-l-adj:length(v)+l]
	return [dot(extended_v[i-l-adj:i+l],k) for i in 1+l+adj:length(extended_v)-l]
end

# ╔═╡ 9d20cc00-1488-11eb-12c2-7dd0d2f335c8
convolve_vector([1, 10, 100, 1000, 10000],[0,1,0])

# ╔═╡ b8416724-148d-11eb-1bb0-ffeb06450e7b
3%2

# ╔═╡ b73f85f4-148d-11eb-2934-7d3a3d7d7c86
3÷2
...
# ╔═╡ 822fdccc-148a-11eb-2783-8b80beb7a904
gaussian_kernel(5)

# ╔═╡ f8bd22b8-ee14-11ea-04aa-ab16fd01826e
md"Let's test your kernel function!"

# ╔═╡ f8aaa80e-148b-11eb-23fb-cf574218763c
@bind gaussian_kernel_size_1D html"<input type=range>"

# ╔═╡ 2a9dd06a-ee13-11ea-3f84-67bb309c77a8
gaussian_kernel_size_1D # change this value, or turn me into a slider!
...
function extend_mat(M::AbstractMatrix, i, j)
	n_row, n_col = size(M)
	r_idx, j_idx = i, j  # index of closest elements
	i <1 ? r_idx = 1 : r_idx = min(n_row, i)
	j <1 ? n_idx = 1 : n_idx = min(n_col, j)
	return M[r_idx, n_idx]
end
...
# ╔═╡ f41fc8c2-14a0-11eb-1c70-75d484d5738d
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	K = parent(K) # deal with offset arrays
	k_rows, k_cols = size(K)  # size of the window
	m_rows, m_cols = size(M)  # size of M
	l_rows, l_cols = (k_rows-1)÷2, (k_cols-1)÷2  # required padding around i,j
	k_rows % l_rows == 1 ? r_adj = 1 : r_adj = 0 # if window rows doesn't match K
	k_cols % l_cols == 1 ? c_adj = 1 : c_adj = 0 # if window cols doesn't match K
	# extend M by l_rows, l_cols in all directions - TODO account for adj
	extend_M = [extend_mat(M, i, j) for (i,j) in Iterators.product(1-l_rows-r_adj:m_rows+l_rows, 1-l_cols-c_adj:m_cols+l_cols)]
	# march K across M iterating over a path that accounts for padding
	path = [(i,j) for (i,j) in Iterators.product(1+l_rows+r_adj:m_rows-l_rows, 1+l_cols+c_adj:m_cols-l_cols)]

	return [sum(extend_M[i-l_rows-r_adj:i+l_rows, j-l_cols-c_adj:j+l_cols] .*K) for (i,j) in path]
end

# ╔═╡ eece8032-14b1-11eb-0c14-436275b3abb5
philip_head = philip[250:430,110:230]

# ╔═╡ dfc4daa6-149c-11eb-168b-a94f1b15c76f
...
# ╔═╡ 1dc7c298-14b4-11eb-3276-21c3fec06bc9
parent(K_test)

# ╔═╡ 84d60c52-14b3-11eb-33f2-a189d180be50
K_test, typeof(K_test), size(K_test)

# ╔═╡ 2aa1c8aa-1497-11eb-1f85-6ba3cefbf980
...
# ╔═╡ 583dfc06-14a7-11eb-14fc-399b8f60a422
K1, K2=Kernel.sobel()

# ╔═╡ 84e42094-149a-11eb-2dcd-e757f90ec076

begin  # test
	
	M, K = philip[250:430,110:230], parent(K2)
	k_rows, k_cols = size(K)  # size of the window
	m_rows, m_cols = size(M)  # size of M
	l_rows, l_cols = (k_rows-1)÷2, (k_cols-1)÷2  # required padding around i,j
	
	# extend M by l_rows, l_cols in all directions - TODO account for adj
	extend_M = [extend_mat(M, i, j) for (i,j) in Iterators.product(1-l_rows:m_rows+l_rows, 1-l_cols:m_cols+l_cols)]
	path = [(i,j) for (i,j) in Iterators.product(1+l_rows:m_rows-l_rows, 1+l_cols:m_cols-l_cols)]
	result = [sum(extend_M[i-l_rows:i+l_rows, j-l_cols:j+l_cols] .*K) for (i,j) in path]
end


# ╔═╡ aab92dd0-14a1-11eb-3acc-7977411373f8
i2, j2 = path[1]

# ╔═╡ 3e3a64a8-14a6-11eb-1dc1-27c01724d67b
M3=extend_M[i2-l_rows:i2+l_rows, j2-l_cols:j2+l_cols]

# ╔═╡ 32c8ebb4-14b3-11eb-2dff-b5c93d7bc753
typeof(M3)

# ╔═╡ fe5f3b50-14b2-11eb-3ae0-9b4db2575b8c
size(K)

# ╔═╡ 1ec51348-149c-11eb-0a14-858d211befa3
begin
	i, j = path[50]
	window = extend_M[i-l_rows:i+l_rows,j-l_cols:j+l_cols]
	window
end

# ╔═╡ d5070b68-149a-11eb-0214-a551e81307aa
size(M), size(K)

# ╔═╡ 38f2f098-14b3-11eb-0313-4100b1338a19
K2, typeof(K2), size(K2)

# ╔═╡ 9acaf07e-14a7-11eb-055a-cf31fdb6d545
convolve_image(philip, K1)*5

# ╔═╡ 49a37316-14b5-11eb-0322-4d09821bc1e8
convolve_image(philip, K2)*5

# ╔═╡ 937cf820-14af-11eb-2de6-ffbf374758ff
begin
	Kg = parent(Kernel.gaussian(1))
	Kg
end
...
# ╔═╡ 2f609c7c-14d3-11eb-234a-cfd643a3fab9
function gaussian(x::Number, y::Number, stdev::Float64)
	n = (2*pi*stdev^2)^-1
	return n* exp(-(x^2+y^2)/(2*(stdev)^2))
end

# ╔═╡ 5edc0d1a-14d3-11eb-380d-c113b191c596
gaussian(0,0,5.5) #https://softwarebydefault.com/2013/06/08/calculating-gaussian-kernels/

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	s = 1.  #standard deviation
	K = [gaussian(1,1,s) gaussian(0,1,s) gaussian(1,1,s);
	gaussian(1,0,s) gaussian(0,0,s) gaussian(1,0,s);
	gaussian(1,1,s) gaussian(0,1,s) gaussian(1,1,s);]
	k_total = sum(K)
	K = K /k_total
	return convolve_image(image, K)
end

# ╔═╡ 6f8b393c-14b1-11eb-0cae-e1c085802150
with_gaussian_blur(philip_head)

# ╔═╡ 6f09c168-14d4-11eb-2e22-1d8d9d03caa9
